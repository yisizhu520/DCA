...
class Element implements Comparable<Element> {
    ...
    public boolean equals(Object o) {
        ...
        boolean conditionSame = true;
        for (int j = 0; j < valueOfCondition.size(); j++) {
            if (!valueOfCondition.get(j).equals(element.valueOfCondition.get(j))) {
                conditionSame = false;
                break;
            }
        }
        return conditionSame && valueOfDecision == element.valueOfDecision;
    }

    @Override
    public int hashCode() {
        return Objects.hash(valueOfCondition, valueOfDecision);
    }
